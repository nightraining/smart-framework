...
public class WebUtil {
    ...
    /**
     * 将数据以 HTML 格式写入响应中（在 JS 中获取的是 JOSN 字符串，而不是 JSON 对象）
     * @param response
     * @param data
     */
    public static void WriteHTML(HttpServletResponse response,Object data){
        try {
            ...
            //向响应中写入数据
            PrintWriter writer = response.getWriter();
            writer.write(JsonUtil.toJSON(data)); //转为 JOSN 字符串
            writer.flush();
            writer.close();
        }catch (Exception e){
            logger.error("在响应中写数据出错！", e);
                            ...
                            if (ArrayUtil.isNotEmpty(array) && array.length == 2){
                                ...
                                if (checkParamName(paramName)){
                                    if (paramMap.containsKey(paramName)){
                                        paramValue = paramMap.get(paramName)+StringUtil.SEPERATOR+paramValue;
                                    }
                            ...
                            if (paramValues.length == 1){
                                ...
                                for (int i = 0; i < paramValues.length; i++){
                                    ...
                                    if (i != paramValues.length - 1){
                                        paramValue.append(StringUtil.SEPERATOR);
                                    }
    ...
    public static String createCaptcha(HttpServletResponse response){
        ...
        try {
            //参数初始化
            int width = 60;                       //验证码图片宽度
            int height = 25;                      //验证码图片高度
            int codeCount = 4;                    //验证码字符个数
            int codeX = width / (codeCount + 1);  //字符横向间距
            int cideY = height - 4;               //字符纵向间距
            ...
            for (int i = 0; i < codeCount; i++){
                ...
                g.drawString(validateCode,(i + 1) * codeX - 6, cideY);
                //将产生的随机数拼接起来
    ...
    public boolean isIE(HttpServletRequest request){
        ...
        return agent != null && agent.contains("HSIE");
    }
}
