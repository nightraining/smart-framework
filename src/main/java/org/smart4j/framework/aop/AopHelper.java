...
import org.smart4j.framework.core.dault.InitializationError;
...
public class AopHelper {
    ...
    private static void addPluginProxy(Map<Class<?>, List<Class<?>>> proxyMap) throws Exception{
        // 获取插件包名写父类为 PluginProxy 的所有类（插件代理类）
        List<Class<?>> pluginProxyClassList = classScanner.getClassListBySuper(FrameworkConstant.PLUGIN_PACEAGE, PluginProxy.class);
    ...
    private static void addAspectProxy(Map<Class<?>, List<Class<?>>> proxyMap) {
        // 获取切面类（所有继承于 AspectProxy 类）
        List<Class<?>> aspectProxyClassList = ClassHelper.getClassListBySuper(AspectProxy.class);
        // 添加插件包下所有的切面类
        aspectProxyClassList.addAll(classScanner.getClassListBySuper(FrameworkConstant.PLUGIN_PACEAGE, AspectProxy.class));
        // 排序切面类
    ...
    private static List<Class<?>> createTargetClassList(Aspect aspect) {
        ...
        if (StringUtil.isNotEmpty(pkg)){
            ...
            // 若注解不为空且不是 Aspect 注解，则添加应用包名下所有带该注解的所有类
            if (annotation != null && !annotation.equals(Aspect.class)){
