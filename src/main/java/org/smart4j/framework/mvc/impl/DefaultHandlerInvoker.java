...
public class DefaultHandlerInvoker implements HandlerInvoker {
    ...
    public List<Object> createActionMethodParamList(HttpServletRequest request, Handler handler) throws  Exception{
        ...
        if (UploadHelper.isMultipart(request)){
            ...
            // 添加普通请求参数列表（包括Query String 与 Form Data）
     ...
     * @return
     */
    private Collection<?> createPathParamList(Matcher requestPathMatcher, Class<?>[] actionParamTypes) {
        // 定义参数列表
        List<Object> paramList = new ArrayList<Object>();
        // 遍历正则表达式中所匹配的组
        for (int i=0; i<=requestPathMatcher.groupCount(); i++){
            ...
            // 获取请求参数类型（支持四中类型：int/Integer、long/Long、double/Double、String）
    ...
    private void checkParamList(Method actionMethod, List<Object> actionMethodParamList) {
        ...
        if (actionMethodParameterTypes.length != actionMethodParamList.size()){
            String message = String.format("因为参数个数不匹配，所以无法调用Action方法！原始参数个数：%d，实际参数个数：%d",actionMethodParameterTypes.length,actionMethodParamList.size());
    ...
    private Object invokeActionMethod(Method actionMethod, Object actionInstance, List<Object> actionMethodParamList) throws Exception {
        // 通过反射调用 Action 方法
        actionMethod.setAccessible(true);  //取消类型安全检查（可提高反射性能）
        return actionMethod.invoke(actionInstance, actionMethodParamList);
    }
}
